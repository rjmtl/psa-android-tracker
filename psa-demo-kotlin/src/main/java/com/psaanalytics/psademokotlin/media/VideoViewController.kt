...
package com.psaanalytics.psademokotlin.media
...
import com.psaanalytics.psa.media.controller.MediaTracking
import com.psaanalytics.psa.media.entity.MediaPlayerEntity
import com.psaanalytics.psa.media.event.*
import com.psaanalytics.psademokotlin.R
import java.util.*
...
class VideoViewController(activity: Activity, uri: Uri) {
    ...
    private fun onComplete(player: MediaPlayer) {
        ...
        if (loaded) {
            track(MediaEndEvent())
            mediaTracking?.id?.let { Psa.defaultTracker?.media?.endMediaTracking(it) }
    ...
    private fun load() {
        if (!loaded) {
            ...
            mediaTracking = Psa.defaultTracker?.media?.startMediaTracking(
                configuration
            )
