 ...
 */
package com.example.psa_trakcer.core.session
...
import com.example.psa_trakcer.psa.entity.ClientSessionEntity
import com.example.psa_trakcer.psa.payload.SelfDescribingJson
import com.example.psa_trakcer.psa.tracker.SessionState
import com.example.psa_trakcer.psa.tracker.SessionState.Companion.build
...
class Session @SuppressLint("ApplySharedPref") constructor(
    ...
    init {
        ...
        var sessionVarsName = com.example.psa_trakcer.core.constants.TrackerConstants.SNOWPLOW_SESSION_VARS
        if (namespace != null && namespace.isNotEmpty()) {
            val sessionVarsSuffix = namespace.replace("[^a-zA-Z0-9_]+".toRegex(), "-")
            sessionVarsName = com.example.psa_trakcer.core.constants.TrackerConstants.SNOWPLOW_SESSION_VARS + "_" + sessionVarsSuffix
        }
    ...
    fun getSessionContext(
        ...
        sessionCopy[com.example.psa_trakcer.core.constants.Parameters.SESSION_EVENT_INDEX] = eventIndex
        if (userAnonymisation) {
            sessionCopy[com.example.psa_trakcer.core.constants.Parameters.SESSION_USER_ID] =
                "00000000-0000-0000-0000-000000000000"
            sessionCopy[com.example.psa_trakcer.core.constants.Parameters.SESSION_PREVIOUS_ID] = null
        }
    ...
    private fun storeSessionState(state: SessionState) {
        ...
        editor.putString(com.example.psa_trakcer.core.constants.TrackerConstants.SESSION_STATE, jsonString)
        editor.apply()
    }
    ...
    private fun getSessionMap(
        ...
        try {
            ...
            if (!sharedPreferences.contains(com.example.psa_trakcer.core.constants.TrackerConstants.SESSION_STATE)) {
                return null
            }
            val sessionMap: MutableMap<String?, Any?> = HashMap()
            val jsonString = sharedPreferences.getString(com.example.psa_trakcer.core.constants.TrackerConstants.SESSION_STATE, null)
    ...
    companion object {
        ...
        private fun retrieveUserId(context: Context, state: SessionState?): String {
            ...
            val generalPref = context.getSharedPreferences(
                com.example.psa_trakcer.core.constants.TrackerConstants.SNOWPLOW_GENERAL_VARS,
                Context.MODE_PRIVATE
            )
            val storedUserId = generalPref.getString(com.example.psa_trakcer.core.constants.TrackerConstants.INSTALLATION_USER_ID, null)
            if (storedUserId != null) {
                userId = storedUserId
            } else {
                generalPref.edit()
                    .putString(com.example.psa_trakcer.core.constants.TrackerConstants.INSTALLATION_USER_ID, userId)
                    .commit()
            }
            return userId
        }
