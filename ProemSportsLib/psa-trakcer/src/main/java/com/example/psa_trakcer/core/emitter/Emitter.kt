...
import com.example.psa_trakcer.core.emitter.storage.SQLiteEventStore
import com.example.psa_trakcer.core.tracker.Logger
import com.example.psa_trakcer.core.utils.Util
import com.example.psa_trakcer.psa.emitter.BufferOption
import com.example.psa_trakcer.psa.emitter.EmitterEvent
import com.example.psa_trakcer.psa.emitter.EventStore
import com.example.psa_trakcer.psa.network.HttpMethod
import com.example.psa_trakcer.psa.network.NetworkConnection
import com.example.psa_trakcer.psa.network.OkHttpNetworkConnection
import com.example.psa_trakcer.psa.network.OkHttpNetworkConnection.OkHttpNetworkConnectionBuilder
import com.example.psa_trakcer.psa.network.Protocol
import com.example.psa_trakcer.psa.network.Request
...
class Emitter(
    ...
    context: Context,
    collectorUri: String,
    builder: ((com.example.psa_trakcer.core.emitter.Emitter) -> Unit)? = null
) {
    private val TAG = com.example.psa_trakcer.core.emitter.Emitter::class.java.simpleName
     ...
     */
    var timeUnit: TimeUnit = com.example.psa_trakcer.core.emitter.EmitterDefaults.timeUnit
        set(unit) {
            if (!builderFinished) {
                field = unit
            }
    ...
    var threadPoolSize = com.example.psa_trakcer.core.emitter.EmitterDefaults.threadPoolSize
        set(poolSize) {
            if (!builderFinished) {
                field = poolSize
            }
     ...
     */
    var tlsVersions: EnumSet<com.example.psa_trakcer.core.emitter.TLSVersion> =
        com.example.psa_trakcer.core.emitter.EmitterDefaults.tlsVersions
     ...
     */
    var emitterTick: Int = com.example.psa_trakcer.core.emitter.EmitterDefaults.emitterTick
     ...
     */
    var emptyLimit: Int = com.example.psa_trakcer.core.emitter.EmitterDefaults.emptyLimit

    /**
     * The maximum amount of events to grab for an emit attempt.
     */
    var emitRange: Int = com.example.psa_trakcer.core.emitter.EmitterDefaults.emitRange

    /**
     * The GET byte limit
     */
    var byteLimitGet: Long = com.example.psa_trakcer.core.emitter.EmitterDefaults.byteLimitGet

    /**
     * The POST byte limit
     */
    var byteLimitPost: Long = com.example.psa_trakcer.core.emitter.EmitterDefaults.byteLimitPost
     ...
     */
    var httpMethod: HttpMethod = com.example.psa_trakcer.core.emitter.EmitterDefaults.httpMethod
        /**
    ...
    var bufferOption: BufferOption =
        com.example.psa_trakcer.core.emitter.EmitterDefaults.bufferOption
        /**
     ...
     */
    var requestSecurity: Protocol =
        com.example.psa_trakcer.core.emitter.EmitterDefaults.httpProtocol
        /**
     ...
     */
    var emitTimeout: Int? = com.example.psa_trakcer.core.emitter.EmitterDefaults.emitTimeout
        set(emitTimeout) {
            emitTimeout?.let {
                field = emitTimeout
                if (!isCustomNetworkConnection && builderFinished) {
    ...
    var serverAnonymisation: Boolean =
        com.example.psa_trakcer.core.emitter.EmitterDefaults.serverAnonymisation
        /**
    ...
    private val _retryFailedRequests = AtomicReference(com.example.psa_trakcer.core.emitter.EmitterDefaults.retryFailedRequests)
    /**
    ...
    var maxEventStoreSize: Long =
        com.example.psa_trakcer.core.emitter.EmitterDefaults.maxEventStoreSize
    ...
    var maxEventStoreAge: Duration =
        com.example.psa_trakcer.core.emitter.EmitterDefaults.maxEventStoreAge
    ...
    init {
        ...
        if (threadPoolSize > 2) {
            com.example.psa_trakcer.core.emitter.Executor.threadCount = threadPoolSize
        }
    ...
    fun add(payload: Payload) {
        com.example.psa_trakcer.core.emitter.Executor.execute(TAG) {
    ...
    fun shutdown(timeout: Long): Boolean {
        ...
        val es = com.example.psa_trakcer.core.emitter.Executor.shutdown()
    ...
    private fun isOversize(
        ...
        val wrapperBytes =
            if (previousPayloads.isNotEmpty()) previousPayloads.size + com.example.psa_trakcer.core.emitter.Emitter.Companion.POST_WRAPPER_BYTES else 0
    ...
    private fun addSendingTimeToPayload(payload: Payload, timestamp: String) {
        payload.add(com.example.psa_trakcer.core.constants.Parameters.SENT_TIMESTAMP, timestamp)
    }
