...
import com.example.psa_trakcer.BuildConfig
import com.example.psa_trakcer.core.emitter.Executor.execute
import com.example.psa_trakcer.core.gdpr.Gdpr
import com.example.psa_trakcer.core.screenviews.ScreenState
import com.example.psa_trakcer.core.screenviews.ScreenStateMachine
import com.example.psa_trakcer.core.screenviews.ScreenSummaryStateMachine
import com.example.psa_trakcer.core.session.ProcessObserver.Companion.initialize
import com.example.psa_trakcer.core.session.Session
import com.example.psa_trakcer.core.session.Session.Companion.getInstance
import com.example.psa_trakcer.core.statemachine.DeepLinkStateMachine
import com.example.psa_trakcer.core.statemachine.LifecycleStateMachine
import com.example.psa_trakcer.core.statemachine.StateMachineInterface
import com.example.psa_trakcer.core.statemachine.StateManager
import com.example.psa_trakcer.core.tracker.Logger.d
import com.example.psa_trakcer.core.tracker.Logger.track
import com.example.psa_trakcer.core.tracker.Logger.updateLogLevel
import com.example.psa_trakcer.core.tracker.Logger.v
import com.example.psa_trakcer.core.utils.NotificationCenter.FunctionalObserver
import com.example.psa_trakcer.core.utils.NotificationCenter.addObserver
import com.example.psa_trakcer.core.utils.NotificationCenter.removeObserver
import com.example.psa_trakcer.core.utils.Util.getApplicationContext
import com.example.psa_trakcer.core.utils.Util.getGeoLocationContext
import com.example.psa_trakcer.core.utils.Util.truncateUrlScheme
import com.example.psa_trakcer.psa.configuration.PlatformContextProperty
import com.example.psa_trakcer.psa.entity.DeepLink
import com.example.psa_trakcer.psa.event.Background
import com.example.psa_trakcer.psa.event.DeepLinkReceived
import com.example.psa_trakcer.psa.event.Event
import com.example.psa_trakcer.psa.event.Foreground
...
import com.example.psa_trakcer.psa.tracker.LoggerDelegate
...
class Tracker(
    emitter: com.example.psa_trakcer.core.emitter.Emitter,
    ...
    var emitter: com.example.psa_trakcer.core.emitter.Emitter = emitter
        set(emitter) {
    ...
    */
    ...
    private fun setApplicationInstallEventTimestamp(event: TrackerEvent) {
        ...
        if (event.schema != null && event.schema == com.example.psa_trakcer.core.constants.TrackerConstants.SCHEMA_APPLICATION_INSTALL) {
    ...
    private fun addBasicPropertiesToPayload(payload: Payload, event: TrackerEvent) {
        // Event ID
        payload.add(com.example.psa_trakcer.core.constants.Parameters.EID, event.eventId.toString())
        // Timestamps
        payload.add(com.example.psa_trakcer.core.constants.Parameters.DEVICE_TIMESTAMP, event.timestamp.toString())
        event.trueTimestamp?.let { payload.add(com.example.psa_trakcer.core.constants.Parameters.TRUE_TIMESTAMP, it.toString()) }
        // Tracker info (version, namespace, app ID)
        payload.add(com.example.psa_trakcer.core.constants.Parameters.APPID, appId)
        payload.add(com.example.psa_trakcer.core.constants.Parameters.NAMESPACE, namespace)
        payload.add(com.example.psa_trakcer.core.constants.Parameters.TRACKER_VERSION, trackerVersion)
        // Subject
        subject?.let { payload.addMap(HashMap(it.getSubject(userAnonymisation))) }
        // Platform
        payload.add(com.example.psa_trakcer.core.constants.Parameters.PLATFORM, platform.value)
        // Event name
        if (event.isPrimitive) {
            payload.add(com.example.psa_trakcer.core.constants.Parameters.EVENT, event.name)
        } else {
            payload.add(com.example.psa_trakcer.core.constants.Parameters.EVENT, com.example.psa_trakcer.core.constants.TrackerConstants.EVENT_UNSTRUCTURED)
        }
    }
    ...
    */
    private fun workaroundForCampaignAttributionEnrichment(payload: Payload, event: TrackerEvent) {
        ...
        } else if (event.schema == com.example.psa_trakcer.core.constants.TrackerConstants.SCHEMA_SCREEN_VIEW) {
        ...
        if (url != null) {
            payload.add(com.example.psa_trakcer.core.constants.Parameters.PAGE_URL, truncateUrlScheme(url))
        }
        if (referrer != null) {
            payload.add(com.example.psa_trakcer.core.constants.Parameters.PAGE_REFR, truncateUrlScheme(referrer))
        }
    }
