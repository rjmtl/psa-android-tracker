/*
 * Copyright (c) 2015-present PSA Analytics Ltd. All rights reserved.
 *
 * This program is licensed to you under the Apache License Version 2.0,
 * and you may not use this file except in compliance with the Apache License Version 2.0.
 * You may obtain a copy of the Apache License Version 2.0 at http://www.apache.org/licenses/LICENSE-2.0.
 *
 * Unless required by applicable law or agreed to in writing,
 * software distributed under the Apache License Version 2.0 is distributed on an
 * "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the Apache License Version 2.0 for the specific language governing permissions and limitations there under.
 */
package com.example.psa_trakcer.psa.event

import com.example.psa_trakcer.core.constants.Parameters
import com.example.psa_trakcer.core.constants.TrackerConstants

/** An ecommerce item event.  
 * @param sku Stock Keeping Unit of the item.
 * @param price Price of the item.
 * @param quantity Quantity of the item.
 * @Deprecated Use the ecommerce package instead
 */
@Deprecated("Use the ecommerce package instead")
class EcommerceTransactionItem(sku: String, price: Double, quantity: Int) : AbstractPrimitive() {
    /** Stock Keeping Unit of the item.  */
    val sku: String

    /** Price of the item.  */
    val price: Double

    /** Quantity of the item.  */
    val quantity: Int

    /** Name of the item.  */
    var itemName: String? = null

    /** Category of the item.  */
    var category: String? = null

    /** Currency used for the price of the item.  */
    var currency: String? = null

    /** OrderID of the order that contains this item.  */
    var orderId: String? = null

    /**
     * Creates an ecommerce item event.
     */
    init {
        require(sku.isNotEmpty()) { "sku cannot be empty" }
        
        this.sku = sku
        this.price = price
        this.quantity = quantity
    }
    
    // Builder methods
    
    /** Name of the item.  */
    fun name(name: String?): EcommerceTransactionItem {
        this.itemName = name
        return this
    }

    /** Category of the item.  */
    fun category(category: String?): EcommerceTransactionItem {
        this.category = category
        return this
    }

    /** Currency used for the price of the item.  */
    fun currency(currency: String?): EcommerceTransactionItem {
        this.currency = currency
        return this
    }

    /** OrderID of the order that contains this item.  */
    fun orderId(orderId: String?): EcommerceTransactionItem {
        this.orderId = orderId
        return this
    }

    // Public methods
    
    override val dataPayload: Map<String, Any?>
        get() {
            val payload = HashMap<String, Any?>()
            orderId?.let { payload[com.example.psa_trakcer.core.constants.Parameters.TI_ITEM_ID] = it }
            payload[com.example.psa_trakcer.core.constants.Parameters.TI_ITEM_SKU] = sku
            payload[com.example.psa_trakcer.core.constants.Parameters.TI_ITEM_PRICE] = price.toString()
            payload[com.example.psa_trakcer.core.constants.Parameters.TI_ITEM_QUANTITY] = quantity.toString()
            itemName?.let { payload[com.example.psa_trakcer.core.constants.Parameters.TI_ITEM_NAME] = it }
            category?.let { payload[com.example.psa_trakcer.core.constants.Parameters.TI_ITEM_CATEGORY] = it }
            currency?.let { payload[com.example.psa_trakcer.core.constants.Parameters.TI_ITEM_CURRENCY] = it }
            return payload
        }

    override val name: String
        get() = com.example.psa_trakcer.core.constants.TrackerConstants.EVENT_ECOMM_ITEM
}
