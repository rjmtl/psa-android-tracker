    ...
    override fun payloadValues(event: InspectableEvent, state: State?): Map<String, Any>? {
        if (state is ScreenState) {
            ...
            if (value != null && value.isNotEmpty()) {
                addedValues[com.example.psa_trakcer.core.constants.Parameters.SV_PREVIOUS_NAME] = value
            }
            value = state.previousId
            if (value != null && value.isNotEmpty()) {
                addedValues[com.example.psa_trakcer.core.constants.Parameters.SV_PREVIOUS_ID] = value
            }
            value = state.previousType
            if (value != null && value.isNotEmpty()) {
                addedValues[com.example.psa_trakcer.core.constants.Parameters.SV_PREVIOUS_TYPE] = value
            }
            return addedValues
        }
        return null
