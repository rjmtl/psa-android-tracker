...
import com.psaanalytics.core.tracker.SchemaRule.Companion.build
import com.psaanalytics.psa.globalcontexts.SchemaRuleSet.Companion.buildRuleSet
import com.psaanalytics.psa.globalcontexts.SchemaRuleSet.Companion.buildRuleSetWithAllowedList
import com.psaanalytics.psa.globalcontexts.SchemaRuleSet.Companion.buildRuleSetWithDeniedList
...
class SchemaRuleSetTest {
    ...
    fun testSchemaRuleSet() {
        ...
        val psa = "iglu:com.snowplow.*/*/jsonschema/*-*-*"
        val psaTest = "iglu:com.snowplow.test/*/jsonschema/*-*-*"
        val ruleset = buildRuleSet(listOf(acme, psa), listOf(psaTest))
        val allowed = listOf(acme, psa)
        Assert.assertEquals(ruleset.allowed, allowed)
        val denied = listOf(psaTest)
    ...
    fun testSchemaRulesetOnlyDenied() {
        val psaTest = "iglu:com.snowplow.test/*/jsonschema/*-*-*"
        val ruleset = buildRuleSetWithDeniedList(listOf(psaTest))
    ...
    fun testSchemaRulesetOnlyAllowed() {
        val acme = "iglu:com.acme.*/*/jsonschema/*-*-*"
        val psa = "iglu:com.snowplow.*/*/jsonschema/*-*-*"
        val ruleset = buildRuleSetWithAllowedList(listOf(acme, psa))
        val allowed = listOf(acme, psa)
        Assert.assertEquals(ruleset.allowed, allowed)
