...
import com.psaanalytics.core.session.FileStore.deleteFile
import com.psaanalytics.core.session.Session
import com.psaanalytics.core.tracker.Tracker
import com.psaanalytics.core.utils.NotificationCenter.postNotification
...
class SessionTest {
    ...
    fun setUp() {
        val context = context
        cleanSharedPreferences(context, TrackerConstants.PSA_SESSION_VARS)
        cleanSharedPreferences(context, TrackerConstants.PSA_GENERAL_VARS)
    }
    ...
    fun testBackgroundTimeBiggerThanBackgroundTimeoutCausesNewSession() {
        cleanSharedPreferences(context, TrackerConstants.PSA_SESSION_VARS + "_tracker")
    ...
    fun testSessionInitWithIncompletePersistedData() {
        val prefs = context.getSharedPreferences(
            TrackerConstants.PSA_SESSION_VARS,
    ...
    fun testMultipleTrackersUpdateDifferentSessions() {
        cleanSharedPreferences(context, TrackerConstants.PSA_SESSION_VARS + "_tracker1")
        cleanSharedPreferences(context, TrackerConstants.PSA_SESSION_VARS + "_tracker2")
        val emitter = Emitter(
    ...
    private fun getSession(foregroundTimeout: Long, backgroundTimeout: Long): Session {
        context.getSharedPreferences(TrackerConstants.PSA_SESSION_VARS, Context.MODE_PRIVATE)
            .edit()
            .clear()
            .commit()
